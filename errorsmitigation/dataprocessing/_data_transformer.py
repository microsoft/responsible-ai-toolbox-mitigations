...
class DataTransformer():
   ...
   # Split the target and features
    ...
    def _split_features(self):
        try:            
            if(all(isinstance(x, int) for x in self.transform_features)):                
                X_no_transform = self.dataset.drop(self.dataset.columns[self.transform_features],axis=1)                
    ...
    def _transform_data(self, X):
        ...
        elif (self.transformer_type.name == self.TransformerType.QuantileTransformer.name):
            num_pipe = Pipeline([
                ('num_imputer', SimpleImputer(strategy='median')),
