...
def test_one_feature(small_df):
    ...
    agg_measures = AggregateBalanceMeasure(small_df, [feature1])
    expected = {
    ...
    assert agg_measures.measures.iloc[0].to_dict() == pytest.approx(expected)


def test_both_features(small_df):
    feature1 = small_df.columns[1]
    feature2 = small_df.columns[2]
    agg_measures = AggregateBalanceMeasure(small_df, [feature1, feature2])
    expected = {
