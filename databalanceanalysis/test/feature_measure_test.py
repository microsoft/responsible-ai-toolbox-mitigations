...
def test_feature_balance_measures(small_df):
    ...
    feat_measures = FeatureBalanceMeasure(small_df, [feature1], label)
    exp_male_female = {
    ...
    print(feat_measures.measures.head())
    assert feat_measures.measures.iloc[0].to_dict()[
        Measures.DEMOGRAPHIC_PARITY
    ] == pytest.approx(exp_male_female[Measures.DEMOGRAPHIC_PARITY])
