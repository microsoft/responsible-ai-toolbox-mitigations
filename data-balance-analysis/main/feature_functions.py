...
def p_feature(df, sensitive_col_name, feature_value):
    return (df.loc[df[sensitive_col_name] == feature_value]).size[0] / get_total_count(df)


def p_pos_feature(df, sensitive_col_name, label_col_name, feature_value, pos_label_value):
    return df.loc[
...
#     a = np.pow(total_count, 2) * (1 - 2*p_feature - 2*p_pos + 2*p_pos_feature + 2*p_pos*p_feature)
#     b = total_count * (2*p_feature + 2*p_pos - 4*p_pos_feature - 1)
#     c = np.pow(total_count, 2) *np.sqrt((p_feature - np.pow(p_feature, 2)) * (p_pos - np.pow(p_pos,2)))
#     return (a+b)/c
