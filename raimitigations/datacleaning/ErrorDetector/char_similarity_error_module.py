...
class CharSimilarityErrorModule(ErrorModule):
    ...
    # -----------------------------------
    def predict(self, strings: list) -> set:
        """
    ...
    def get_erroneous_rows_in_col(self, erroneous_vals, dataset, col):
        ...
        """
        col_vals = np.array(dataset[col])  # TODO: doublecheck this returns column not row values even with 2d lists which data[row][col], not dataframe[col][row]
        erroneous_indices = []
