...
class DataStandardScaler(DataScaler):
    """
    Concrete class that applies the :class:`~sklearn.preprocessing.StandardScaler` scaler over a given dataset.
    This class uses the :mod:`sklearn`'s implementation of this scaler (:class:`~sklearn.preprocessing.StandardScaler`)
    at its root, but also makes it simpler to apply to a dataset. For
    example, the user can use a dataset with categorical columns and the scaler
    will be applied only to the numerical columns. Also, the user can provide a
    pipeline of scalers, and all of the scalers in the pipeline will be applied
    before the :class:`~sklearn.preprocessing.StandardScaler` scaler. The user can also use a list of transformations
    using other non-scaler classes implemented in this library (feature selection,
    encoding, imputation, etc.).
    For more details on how the :class:`~sklearn.preprocessing.StandardScaler` scales the data, check the official
    documentation from :mod:`sklearn`:
    https://scikit-learn.org/stable/modules/generated/sklearn.preprocessing.StandardScaler.html

    :param scaler_obj: an object from the :class:`~sklearn.preprocessing.StandardScaler` class. This
        :mod:`sklearn` scaler will be used to perform the scaling process. If None, a
        :class:`~sklearn.preprocessing.StandardScaler` is created using default values;
    ...
    # -----------------------------------
    def _set_scaler(self, scaler_obj: StandardScaler):
        if scaler_obj is None:
            scaler_obj = StandardScaler()
        if not isinstance(scaler_obj, StandardScaler):
            raise ValueError(self._get_wrong_obj_error("DataStandardScaler", "StandardScaler"))
