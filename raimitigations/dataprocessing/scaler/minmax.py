...
class DataMinMaxScaler(DataScaler):
    """
    Concrete class that applies the Min Max scaler over a given dataset. This class
    uses the sklearn's implementation of this scaler (MinMaxScaler) at its root, but
    ...
    all of the scalers in the pipeline will be applied before the Min Max scaler.
    The user can also use a list of transformations using other non-scaler classes
    implemented in this library (SeqFeatSelection, EncoderOHE, DataImputer, etc.).
    For more details on how the MinMaxScaler scales the data, check the official
    documentation from sklearn:
    https://scikit-learn.org/stable/modules/generated/sklearn.preprocessing.MinMaxScaler.html

    :param scaler_obj: an object from the MinMaxScaler class. This sklearn
        scaler will be used to perform the scaling process. If None, a MinMaxScaler
    ...
    # -----------------------------------
    def _set_scaler(self, scaler_obj: MinMaxScaler):
        if scaler_obj is None:
            scaler_obj = MinMaxScaler()
        if not isinstance(scaler_obj, MinMaxScaler):
            raise ValueError(self._get_wrong_obj_error("DataMinMaxScaler", "MinMaxScaler"))
