...
class DataNormalizer(DataScaler):
    """
    Concrete class that applies the :class:`~sklearn.preprocessing.Normalizer` scaler over a given dataset. This class
    uses the :mod:`sklearn`'s implementation of this scaler (:class:`~sklearn.preprocessing.Normalizer`) at its root, but
    also makes it more simple to be applied to a dataset. For example, the user
    can use a dataset with categorical columns and the scaler will be applied only
    to the numerical columns. Also, the user can provide a pipeline of scalers, and
    all of the scalers in the pipeline will be applied before the :class:`~sklearn.preprocessing.Normalizer` scaler.
    ...
    For more details on how the :class:`~sklearn.preprocessing.Normalizer` scales the data, check the `official
    documentation from sklearn
    <https://scikit-learn.org/stable/modules/generated/sklearn.preprocessing.Normalizer.html>`_.

    :param scaler_obj: an object from the :class:`~sklearn.preprocessing.Normalizer` class. This :mod:`sklearn`
        scaler will be used to perform the scaling process. If None, a :class:`~sklearn.preprocessing.Normalizer`
    ...
    # -----------------------------------
    def _set_scaler(self, scaler_obj: Normalizer):
        if scaler_obj is None:
            scaler_obj = Normalizer()
        if not isinstance(scaler_obj, Normalizer):
            raise ValueError(self._get_wrong_obj_error("DataNormalizer", "Normalizer"))
